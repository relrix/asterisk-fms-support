...
static struct rtmp_pvt *rtmp_alloc(char *writestream, char *readstream, char *readnum) {
	...
	ast_debug(3, "To RTMP resample init resample \n");
...
static int rtmp_write(struct ast_channel *ast, struct ast_frame *frame)
{
	...
	if (frame->frametype == AST_FRAME_VIDEO) {
		ast_debug(3, "I GOT A VIDEO FRAME, WORKING FOR IT!\n");
		res = rtmp_send_video(p, frame);	
		
	}

	if (frame->frametype == AST_FRAME_VOICE) {
		ast_debug(3, "I GOT A VOICE FRAME, RESAMPLING AND SENDING!\n");
...
static struct ast_channel *rtmp_new(struct rtmp_pvt *p, int state, const char *linkedid)
{
	...
	ast_log(LOG_WARNING, "Lets Create a RTMP channel now\n");
	...
	ast_log(LOG_WARNING, "finding decoding codec\n");
...
static struct ast_channel *rtmp_request(const char *type, format_t format, const struct ast_channel *requestor, void *data, int *cause)
{
	...
	format &= (AST_FORMAT_SLINEAR);
	 ...
	 ast_log(LOG_WARNING, "RTMP request got !!\n");	
	...
	ast_log(LOG_WARNING, "Building new stream, parse : %s\n", parse);	
	ast_log(LOG_WARNING, "writestream : %s\n", args.writestream);
		ast_log(LOG_WARNING,"readstream : %s\n", args.readstream);
			ast_log(LOG_WARNING,"readnum : %s\n", ast_strlen_zero(args.readnum) ? "None" : args.readnum);
...
static int load_module(void) {
	...
	ast_log(LOG_WARNING, "Initializing acodec\n");	

	/* must be called before using avcodec lib */
	avcodec_init();

	 ast_log(LOG_WARNING, "Registering acodecs \n");	
